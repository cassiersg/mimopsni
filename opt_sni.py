...
def opt_sni(g, split_c=tuple(), indep_bits=True, max_seconds=4*60):
    ...
    #prob.solve(pulp.PULP_CBC_CMD(msg=1, threads=32, maxSeconds=max_seconds))
    prob.solve(pulp.solvers.CPLEX_CMD(
        msg=1,
        timelimit=max_seconds,
        path='/home/cassiersg/.local/cplex_128/cplex/bin/x86-64_linux/cplex'
        ))
